...
namespace Web.Controllers.Api
{
    ...
    public class TaskController : ApiController
    {
        ...
        public dynamic ExportToExcel([FromUri]Web.Infrastructure.JqGrid.JqGridSearchModel searchModel)
        {
            ...
            {
                searchModel.rows = 0;
                var data = GetQuery(Util.GetStatelessQuery<ScheduleTask>(),searchModel);
                var dataList = data.Items.Select(x => new { x.Id, x.Name, x.Seconds, x.Type, x.Enabled, x.StopOnError, x.LastStartUtc, x.LastEndUtc, x.LastSuccessUtc }).ToList();
        ...
        private GridModel<ScheduleTask> GetQuery(IQueryable<ScheduleTask> query,[FromUri] JqGridSearchModel searchModel, int maxRecords = Constants.DEFAULT_MAX_RECORDS_RETURN)
        {
