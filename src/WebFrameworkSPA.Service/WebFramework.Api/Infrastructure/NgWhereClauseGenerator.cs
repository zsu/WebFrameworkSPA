...
{
    public class NgWhereClauseGenerator
    {
        ...
        private StringBuilder ParseRule(ICollection<JqGridRule> rules, GroupOperations groupOp, Type targetSearchType)
        {
            ...
            foreach (var rule in rules)
            {
                ...
                //if (!emptyNullable && props[rule.field].IsGenericType && props[rule.field].GetGenericTypeDefinition() == typeof(Nullable<>))
                //{
                //    sb.AppendFormat("({0}!=null && {0}.Value.ToString().Contains(@{1}){F})".Replace("{F}", fmAdd), rule.field, _formatObjects.Count - 1);
                //}
                //else
